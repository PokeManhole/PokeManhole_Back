...
class userService: 
    ...
    def tryLogin(self,email,password):
        ...
        if resurt:
            ...
            if bcrypt.checkpw(password.encode('utf-8'),resurt['password'].encode('utf-8')):
                # print(pp,pp.decode('utf-8'))
                return {"token":jwt.encode(resurt,"fromis", algorithm="HS256"),"id":resurt["id"],"email":resurt["email"],"name":resurt["name"]}
            return 400
        return 404
